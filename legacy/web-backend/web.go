...
func AccountHandler(writer http.ResponseWriter, request *http.Request) {
	...
	acc, err := GetAccount(vars["account"])
...
func VideoHandler(writer http.ResponseWriter, request *http.Request) {
	...
	vid, err := GetVideo(vars["account"], vars["video"])
...
func AccountLikesHandler(writer http.ResponseWriter, request *http.Request) {
	...
	if limitStr == "" {
		limit = AccHistoricDataDefAmount
	} else {
	...
	likes, err := GetLikesHistory(vars["account"], limit)
...
func AccountFollowersHandler(writer http.ResponseWriter, request *http.Request) {
	...
	followers, err := GetFollowersHistory(vars["account"], limit)
...
func getTimeRangeData(account, query string) (TimeRangeData, error) {
	...
	timeRangeData := make([]TimeRangeElement, 0)
	for k, v := range activity {
		timeRangeData = append(timeRangeData, TimeRangeElement{
...
func getAccountVideos(account, query string) ([]*Video, error) {
	db, err := sql.Open("mysql", DbInfo+"?parseTime=true")
	if err != nil {
		return nil, err
	}
